...
class Display(object):
    def __init__(self,master,img):
        ...
        canvas_width = 850
        canvas_height = 600
        self.color = ["#73ff84", "#ff72f6", "#ffed4a", "#ff6875", "#debcc4", "#000000"]
        self.canvas = Canvas(master, width = canvas_width, height = canvas_height, bg = "#698eb1")
        ...
        w.pack()
        w.place(x=745, y=225)
        w.config(bg='#add8e6',width=5)
        
        #Start Game
        self.startGame = Button(self.canvas, text="Start", background='#add8e6', command = self.startGame, font=("Helvetica"))
    ...
    def startGame(self):
        if(self.num_player == "Players"):
            ...
            #Button
            self.diceRoll = Button(self.canvas, text="Roll",background='#add8e6',
                                   command = self.gamePlay, font=("Helvetica"))
            self.num_player = int(self.num_player)
            self.diceRoll.place(x=760, y=165)
            self.create_peice()
